...
def approxsgd(M, gamma_0,N_0,m_0, x_0, beta, rho, r, t, B,mu):
    ...
    for i in range(1,t+1):
        ...
        h= (d**(-3/2))*(i**(-rho))
        for j in range(int(np.ceil(N_0*(i**(r))))):
            ...
            S,runcomplex=generateS(int(np.ceil(4*np.log(m_0*(i**rho))/np.log(M))),M,B,mu)
            Sreg=S-S1*np.ones(d)
        ...
        optimal=np.array([0.1676141,  0.12145978, 0.15933705, 0.18361757, 0.14116673])
